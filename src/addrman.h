...
public:
    ...
    int GetTriedBucket(const std::vector<unsigned char> &nKey) const;                         // Calculate in which "tried" bucket this entry belongs
    int   GetNewBucket(const std::vector<unsigned char> &nKey, const CNetAddr &src) const;    // Calculate in which "new" bucket this entry belongs, given a certain source
    ...
    bool IsTerrible(int64_t nNow = bitsystem::GetAdjustedTime()) const;
...
#ifdef DEBUG_ADDRMAN
    int Check_();    // Perform consistency check. Returns an error code or zero.
#endif
...
public:
    ...
    void Check() {
#ifdef DEBUG_ADDRMAN
        {
            ...
            int err;
            if ((err = Check_())) {
                logging::LogPrintf("ADDRMAN CONSISTENCY CHECK FAILED!!! err=%i\n", err);
            }
        }
#endif
    }
    ...
    bool Add(const CAddress &addr, const CNetAddr &source, int64_t nTimePenalty = 0) {
        ...
        if (fRet) {
            logging::LogPrintf("Added %s from %s: %i tried, %i new\n", addr.ToStringIPPort().c_str(), source.ToString().c_str(), nTried, nNew);
        }
        return fRet;
    }
    ...
    bool Add(const std::vector<CAddress> &vAddr, const CNetAddr &source, int64_t nTimePenalty = 0) {
        ...
        if (nAdd) {
            logging::LogPrintf("Added %i addresses from %s: %i tried, %i new\n", nAdd, source.ToString().c_str(), nTried, nNew);
        }
        return nAdd > 0;
    }

    // Mark an entry as accessible.
    void Good(const CService &addr, int64_t nTime = bitsystem::GetAdjustedTime()) {
        {
            LOCK(cs);
            Check();
            Good_(addr, nTime);
    ...
    void Attempt(const CService &addr, int64_t nTime = bitsystem::GetAdjustedTime()) {
        {
            LOCK(cs);
            Check();
            Attempt_(addr, nTime);
    ...
    // Mark an entry as currently-connected-to.
    void Connected(const CService &addr, int64_t nTime = bitsystem::GetAdjustedTime()) {
        {
            LOCK(cs);
            Check();
