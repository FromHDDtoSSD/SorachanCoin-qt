...
json_spirit::Value CRPCTable::listunspent(const json_spirit::Array &params, bool fHelp)
{
    ...
    if (params.size() > 2) {
        ...
        for(json_spirit::Value &input: inputs)
        {
            ...
            if (! address.IsValid()) {
                throw bitjson::JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string(("Invalid " + std::string(strCoinName) + " address: ").c_str()) + input.get_str());
            }
...
json_spirit::Value CRPCTable::createrawtransaction(const json_spirit::Array &params, bool fHelp)
{
    ...
    if (params.size() == 3) {
        ...
        CScript scriptPubKey;
        scriptPubKey << ScriptOpcodes::OP_RETURN << strenc::ParseHex(params[2].get_str());
...
json_spirit::Value CRPCTable::decoderawtransaction(const json_spirit::Array &params, bool fHelp)
{
    ...
    rpctable_vector txData(strenc::ParseHex(params[0].get_str()));
