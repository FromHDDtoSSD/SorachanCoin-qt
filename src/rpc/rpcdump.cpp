...
json_spirit::Value CRPCTable::importaddress(const json_spirit::Array &params, bool fHelp)
{
    ...
    } else if (strenc::IsHex(params[0].get_str())) {
        script_vector data(strenc::ParseHex(params[0].get_str()));
        script = CScript(data.begin(), data.end());
    } else {
        throw bitjson::JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, ("Invalid " + std::string(strCoinName) + " address or script").c_str());
    }
