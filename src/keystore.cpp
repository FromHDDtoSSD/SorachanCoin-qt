...
bool CCryptoKeyStore::CreatePrivKey(const CPubKey &pubKeyVariant, const CPubKey &R, CFirmKey &privKey) const
{
    {
        LOCK(cs_KeyStore);
        if (! IsCrypted()) {
...
{
    {
        ...
        {
            CFirmKey key;
            key.SetSecret(mKey.second.first, mKey.second.second);
