...
int check_all_aes() {
   ...
   for (i = 0; i < sizeof(ctaes_tests) / sizeof(ctaes_tests[0]); i++) {
       ...
       if (memcmp(cipher, ciphered, 16)) {
           debugcs::instance() << "E(key=\"" << test->key << "\", plain=\"" << test->plain << "\") != \"" << test->cipher << "\"" << debugcs::endl();
           fail++;
       }
       if (memcmp(plain, deciphered, 16)) {
           debugcs::instance() << "D(key=\"" << test->key << "\", cipher=\"" << test->cipher << "\") != \"" << test->plain << "\"" << debugcs::endl();
           fail++;
       }
   }
   if (fail == 0) {
       debugcs::instance() << "[OK] AES128 AES192 AES256: All tests successful" << debugcs::endl();
   } else {
       debugcs::instance() << fail << "[FAILURE] tests failed" << debugcs::endl();
   }
   return (fail != 0);
}
