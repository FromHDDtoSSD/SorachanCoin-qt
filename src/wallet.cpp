...
bool CWallet::EncryptWallet(const SecureString &strWalletPassphrase)
{
    ...
    }
    {
        ...
        if (fFileBacked) {
            ...
            if(! pwalletdbEncryption) {
                printf("Encrypting Wallet, memory allocate failure.\n");
                return false;
            }
...

    {
        ...
        if (fFileBacked) {
            ...
            if(! pwalletdbDecryption) {
                printf("Decrypting Wallet, memory allocate failure.\n");
                return false;
            }
...
bool CWallet::SelectCoinsMinConf(int64_t nTargetValue, unsigned int nSpendTime, int nConfMine, int nConfTheirs, std::vector<COutput> vCoins, std::set<std::pair<const CWalletTx *,unsigned int> > &setCoinsRet, int64_t &nValueRet) const
{
    ...
    if (coinLowestLarger.second.first && ((nBest != nTargetValue && nBest < nTargetValue + util::CENT) || coinLowestLarger.first <= nBest)) {
        ...
        if (args_bool::fDebug && map_arg::GetBoolArg("-printpriority")) {
            ...
            {
                if (vfBest[i]) {
                    printf("%s ", bitstr::FormatMoney(vValue[i].first).c_str());
                }
            }
...
{
    {
...

        {
            ...
            if(fFileBacked && pwalletdb == NULL) {
                printf("CommitTransaction() : Error: Transaction memory allocate failure.");
                return false;
            }
