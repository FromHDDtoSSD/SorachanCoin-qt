...
#define IMPLEMENT_MEMORY_READWRITE \
        ...
        stack_ptr<memory> pmem(is_direct() ? new(buf) memory(_union.direct): new(buf) memory(_union.indirect)); \
        pmem->readwtite();
...
#define IMPLEMENT_MEMORY_SHARED_READWRITE \
        std::shared_ptr<memory> pmem(is_direct() ? new(std::nothrow) memory(_union.direct): new(std::nothrow) memory(_union.indirect)); \
        ...
        void change_capacity(size_type new_capacity) {
            ...
            if (new_capacity <= N) {
                if (! is_direct()) {
                    ...
                    dm.readwtite();
                    quantum_lib::secure_mprotect_readwrite(_union.indirect);
                ...
                if (! is_direct()) {
                    ...
                    quantum_lib::secure_mprotect_noaccess(_union.indirect);
                } else {
