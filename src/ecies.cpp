...
int cryptogram::ECDH_KDF_X9_62(unsigned char *out, size_t outlen, const unsigned char *Z, size_t Zlen, const unsigned char *sinfo, size_t sinfolen, const EVP_MD *md)
{
    if (sinfolen > ECDH_KDF_MAX || outlen > ECDH_KDF_MAX || Zlen > ECDH_KDF_MAX) {
        return 0;
    }

    int rv = 1;        // returnF1 OK, 0 ERROR
    ...
    for (unsigned int i = 1;;++i)
    {
        ...
        if (outlen >= mdlen) {
            ...
            outlen -= mdlen;
            if (outlen == 0) {
                break;    // OK just (full data)
            }
            out += mdlen;    // output offset
        } else {
            unsigned char mtmp[EVP_MAX_MD_SIZE] = { 0 };    // remain buffer
...
size_t cryptogram::envelope_key_len(const ies_ctx_t *ctx)
{
    return EVP_CIPHER_key_length(ctx->cipher) + EVP_MD_size(ctx->md);
}

EC_KEY *cryptogram::ecies_key_create(const EC_KEY *user, char *error)
{
    EC_KEY *key = NULL;
    if (! (key = EC_KEY_new())) {
...
unsigned char *cryptogram::prepare_envelope_key(const ies_ctx_t *ctx, cryptogram_t *cryptogram, char *error)
{
    ...
    if (ECDH_compute_key(ktmp, ecdh_key_len, EC_KEY_get0_public_key(ctx->user_key), ephemeral, NULL) != (int)ecdh_key_len) {
    ...
    written_length = EC_POINT_point2oct( EC_KEY_get0_group(ephemeral), EC_KEY_get0_public_key(ephemeral), POINT_CONVERSION_COMPRESSED, cryptogram_key_data(cryptogram), ctx->stored_key_length, NULL);
    if (written_length == 0) {
...
int cryptogram::store_cipher_body(const ies_ctx_t *ctx, const unsigned char *envelope_key, const unsigned char *data, size_t length, cryptogram_t *cryptogram, char *error)
{
    ...
    unsigned char *body = cryptogram_body_data(cryptogram);

    int len_sum = 0;

    ///////////////////////////////////////////////////////////////////////////////////////////////
    int out_len = 0;
    if (EVP_EncryptInit_ex(&cipher, ctx->cipher, NULL, envelope_key, iv) != 1 || EVP_EncryptUpdate(&cipher, body, &out_len, data, length) != 1) {
...
int cryptogram::store_mac_tag(const ies_ctx_t *ctx, const unsigned char *envelope_key, cryptogram_t *cryptogram, char *error)
{
    ...
    if (HMAC_Init_ex(&hmac, envelope_key + key_offset, key_length, ctx->md, NULL) != 1 || HMAC_Update(&hmac, cryptogram_body_data(cryptogram), cryptogram_body_length(cryptogram)) != 1 || HMAC_Final(&hmac, cryptogram_mac_data(cryptogram), &out_len) != 1) {
...
cryptogram_t *cryptogram::ecies_encrypt(const ies_ctx_t *ctx, const unsigned char *data, size_t length, char *error)
{
    if (!ctx || !data || !length) {
    ...
    cryptogram = cryptogram_alloc(ctx->stored_key_length, mac_length, length + (length % block_length ? (block_length - (length % block_length)) : 0));
    if (! cryptogram) {
...
EC_KEY *cryptogram::ecies_key_create_public_octets(EC_KEY *user, unsigned char *octets, size_t length, char *error)
{
    EC_KEY *key = NULL;
    EC_POINT *point = NULL;
    const EC_GROUP *group = NULL;
...
unsigned char *cryptogram::restore_envelope_key(const ies_ctx_t *ctx, const cryptogram_t *cryptogram, char *error)
{
    const size_t key_buf_len = envelope_key_len(ctx);
    ...
    if (ECDH_compute_key(ktmp, ecdh_key_len, EC_KEY_get0_public_key(ephemeral), user_copy, NULL) != (int)ecdh_key_len) {
        SET_OSSL_ERROR("An error occurred while ECDH_compute_key");
        goto err;
    }
...
int cryptogram::verify_mac(const ies_ctx_t *ctx, const cryptogram_t *cryptogram, const unsigned char *envelope_key, char *error)
{
    const size_t key_offset = EVP_CIPHER_key_length(ctx->cipher);
    const size_t key_length = EVP_MD_size(ctx->md);
    const size_t mac_length = cryptogram_mac_length(cryptogram);
...
unsigned char *cryptogram::decrypt_body(const ies_ctx_t *ctx, const cryptogram_t *cryptogram, const unsigned char *envelope_key, size_t *length, char *error)
{
...
unsigned char *cryptogram::ecies_decrypt(const ies_ctx_t *ctx, const cryptogram_t *cryptogram, size_t *length, char *error)
{
    ...
    if ((output = decrypt_body(ctx, cryptogram, envelope_key, length, error)) == NULL) {        // malloc
        goto err;
    }
...
ies_ctx_t *cryptogram::create_context(EC_KEY *user_key)
{
    try {
