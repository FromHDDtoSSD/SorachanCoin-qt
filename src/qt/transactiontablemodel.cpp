...
{
    std::map<const TransactionRecord *, int>::const_iterator it = TransactionRecord::mapConfirmations.find(wtx);
    if(it == TransactionRecord::mapConfirmations.end()) {
        QString error = tr("Open Status Error");
        return error;
    }
    int nNumConf = it->second;
