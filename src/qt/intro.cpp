...
{
    return GUIUtil::boostPathToQString(iofs::GetDefaultDataDir());
}
...
void Intro::pickDataDirectory()
{
    ...
    if(! map_arg::GetArg("-datadir", "").empty()) {
        return;
    }
    ...
    if(!fs::exists(GUIUtil::qstringToBoostPath(dataDir)) || map_arg::GetBoolArg("-choosedatadir", false)) {
        ...
        for ( ; ; )
        {
            ...
            try {
                ...
                QMessageBox::critical(0, tr("SorachanCoin-qt"),
     ...
     */
    if(dataDir != getDefaultDataDirectory()) {
        map_arg::SoftSetArg("-datadir", GUIUtil::qstringToBoostPath(dataDir).string()); // use OS locale for path setting
    }
}
