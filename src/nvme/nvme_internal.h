...
#ifndef LIBNVME_INTERNAL_H
#define LIBNVME_INTERNAL_H
 ...
 */
nvme_static_assert(sizeof(struct nvme_tracker) == 4096, "nvme_tracker is not 4K");
nvme_static_assert((offsetof(struct nvme_tracker, u.sgl) & 7) == 0, "SGL must be Qword aligned");
...
namespace nvme {
    ...
    void nvme_request_completion_poll_cb(void *arg,
                                         const struct nvme_cpl *cpl);

    struct nvme_ctrlr *nvme_ctrlr_attach(struct pci_device *pci_dev,
                                         struct nvme_ctrlr_opts *opts);
    void nvme_ctrlr_detach(struct nvme_ctrlr *ctrlr);

    int nvme_qpair_construct(struct nvme_ctrlr *ctrlr,
                             struct nvme_qpair *qpair,
    ...
    int nvme_ns_construct(struct nvme_ctrlr *ctrlr,
